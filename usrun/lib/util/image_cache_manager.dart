...
//    );
  ...
  static CustomImageProvider getImageData({@required String url}) {
    if (url == null) {
      return CustomImageProvider(R.myIcons.imageDefault, cache: _persistentDiskCache);
    }
  ...
  Future<Uint8List> _loadFromDiskCache(CustomImageProvider key, String uId) async {
    ...
    if (cache.isPersistent) {
      ...
      if (imageData != null) {
        ...
        ByteData bytes = await rootBundle.load(R.myIcons.imageDefault);
        Uint8List rawPath = bytes.buffer.asUint8List();
        return rawPath;
      }
    }
